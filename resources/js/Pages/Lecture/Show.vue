<script setup>
  ...
  import { mdiMessageText, mdiSquareEditOutline, mdiTrashCan, mdiAlertCircle } from '@mdi/js'
  ...
  const tab = ref('lectureInformation')
...
<template>
  <PageSection
    :title="props.lecture.lecture_name + ' / ' + props.lecture.professor_name"
    :icon="mdiMessageText"
  >
    <v-tabs
      v-model="tab"
      ...
      <v-window-item value="lectureInformation">
        <v-table density="compact">
          <tbody>
            ...
            <tr><th>講義区分</th><td>{{ props.category.name  }}</td></tr>
            <tr><th>開講学部</th><td>{{ props.faculty ? faculty.name : '未設定' }}</td></tr>
            <tr><th>開講学科・課程</th><td>{{ props.department ? department.name : '未設定' }}</td></tr>
            <tr><th>開講コース・専修</th><td>{{ props.course ? course.name : '未設定'}}</td></tr>
              ...
              >
                ...
                <template v-slot:text>
                  <v-row justify="center" align="center">
                    <v-col cols="5" md="4" class="py-1 py-md-2">
                      <div class="text-medium-emphasis">
                        <span>充実度評価</span>
                        <span>{{ '(' + review.fulfillment_rate + ')' }}</span>
                      </div>
                      <v-rating
                        :model-value="review.fulfillment_rate"
                        size="22"
                        color="primary"
                        density="compact"
                        half-increments
                    ...
                    <v-col cols="5" md="4" class="py-1 py-md-4">
                      <div class="text-medium-emphasis">
                        <span>楽単度評価</span>
                        <span>{{ '(' + review.ease_rate + ')' }}</span>
                      </div>
                      <v-rating
                        :model-value="review.ease_rate"
                        size="22"
                        color="primary"
                        density="compact"
                        half-increments
                    ...
                    <v-col cols="5" md="4" class="py-1 py-md-4">
                      <div class="text-medium-emphasis">
                        <span>満足度評価</span>
                        <span>{{ '(' + review.satisfaction_rate + ')' }}</span>
                      </div>
                      <v-rating
                        :model-value="review.satisfaction_rate"
                        size="22"
                        color="primary"
                        density="compact"
                        half-increments
