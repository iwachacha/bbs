<script setup>
  ...
  const tagSearch = (name) => {
    router.get(route('admin.review.index', { tag: name }), {}, {
      onSuccess: () => {
  ...
  const deleteReview = () => {
    router.delete(route('admin.review.delete', deleteId.value), {
      preserveScroll: true,
      onSuccess: () => {
        ...
        <v-col cols="12" sm="10" md="6" class="py-2 py-sm-3">
          ...
          >
            ...
            <template v-slot:cardTitle>
              <Link :href="route('admin.lecture.show', [review.lecture_id, review.id])">
